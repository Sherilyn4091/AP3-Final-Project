{{--
    ...
    ============================================================================
    Features:
    - CRUD operations for payment statuses (Pending, Paid, Cancelled, etc.)
    - Statistics cards (Total, Active, Most Used)
    - Search and filter functionality
    - Usage checking before deletion
    - Toggle active/inactive status
    - Simple record-keeping labels only (no payment processing)
    ============================================================================
--}}
...
<main class="lg:ml-64 min-h-screen bg-light-gray">
    ...
    <header class="bg-white shadow-sm p-6 border-b-4 border-secondary-blue">
        <div class="flex items-center justify-between flex-wrap gap-4">
            <div>
                <h1 class="text-3xl font-bold text-primary-dark">Payment status management</h1>
                <p class="text-secondary-blue mt-1">Manage payment status labels for record-keeping</p>
            </div>
    ...
    <div class="p-4 lg:p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div class="card p-6 border-l-4 border-secondary-blue">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-600 font-semibold">Total statuses</p>
                    ...
                    <div class="bg-secondary-blue bg-opacity-20 p-3 rounded-full">
                        <svg class="w-8 h-8 text-secondary-blue" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    </div>
                </div>
            </div>
            ...
            <div class="card p-6 border-l-4 border-golden-yellow">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-600 font-semibold">Most used</p>
                        <p class="text-lg font-bold text-primary-dark mt-1">{{ $stats['most_used_name'] }}</p>
                        <p class="text-xs text-gray-500">{{ $stats['most_used_count'] }} {{ $stats['most_used_count'] === 1 ? 'payment' : 'payments' }}</p>
                    </div>
        ...
        <div class="card p-6 mb-6">
            <form method="GET" action="{{ route('admin.payment-statuses.index') }}" class="flex flex-wrap gap-4">
                ...
                <div class="flex items-end gap-2">
                    ...
                    <a href="{{ route('admin.payment-statuses.index') }}" class="bg-gray-200 text-gray-700 px-6 py-2 rounded-lg font-semibold hover:bg-gray-300 transition-all">
                        Clear
                    </a>
                </div>
            </form>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-primary-dark">
                        <tr>
                            <th class="px-6 py-4 text-left text-xs font-bold text-accent-yellow uppercase tracking-wider">Status name</th>
                            <th class="px-6 py-4 text-left text-xs font-bold text-accent-yellow uppercase tracking-wider">Usage</th>
                            <th class="px-6 py-4 text-left text-xs font-bold text-accent-yellow uppercase tracking-wider">Active</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-accent-yellow-light transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    ...
                                    <div class="ml-4">
                                        <div class="text-sm font-bold text-gray-900">{{ $status->status_name }}</div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold {{ $status->is_active ? 'bg-forest-green text-white' : 'bg-gray-400 text-white' }}">
                                    {{ $status->is_active ? 'Active' : 'Inactive' }}
                                </span>
                            </td>
                            
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                {{ date('M d, Y', strtotime($status->created_at)) }}
                            </td>
                            
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <div class="flex gap-2">
                                    <button onclick="editStatus({{ $status->status_id }})" class="action-btn text-secondary-blue">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                        <span class="tooltip">Edit</span>
                                    </button>
                                    
                                    <button onclick="toggleStatus({{ $status->status_id }})" class="action-btn {{ $status->is_active ? 'text-warm-coral' : 'text-forest-green' }}">
                                    ...
                                    <button onclick="deleteStatus({{ $status->status_id }})" class="action-btn text-red-600">
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center text-gray-500">
                                <svg class="w-16 h-16 mx-auto text-gray-300 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                <p class="text-lg font-semibold">No payment statuses found</p>
                                <p class="text-sm mt-2">Try adjusting your filters or add a new status</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
